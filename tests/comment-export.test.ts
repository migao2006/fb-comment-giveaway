...
describe('main comment export', () => {
  it('keeps only main comments even if parsed replies are present', () => {
    expect(orderedComments({ comments: [main], replies: [reply] }).map((comment) => comment.id)).toEqual(['main']);
  });
  ...
  it('writes Excel-friendly CSV with quoting, BOM, and formula protection', () => {
    ...
    expect(csv).not.toContain('"小美"');
    expect(csv).not.toContain('"回覆"');
    expect(csv).not.toContain('"回覆對象"');
    expect(commentsToCsv([{ ...main, authorName: '\t=仍是公式' }])).toContain('"\'\t=仍是公式"');
  });

  it('persists snapshot verification metadata in partial exports', () => {
    const metadata = {
      snapshotId: '12', verificationStatus: 'visible-complete' as const, parsedTotal: 171,
    };
    const csv = commentsToCsv([main], metadata);
    expect(csv).toContain('"資料快照","驗證狀態","實際讀取總數"');
    expect(csv).toContain('"12","visible-complete","171"');
  });
});
