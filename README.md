...
不用安裝 App 或瀏覽器擴充套件。在 Facebook 一般貼文頁執行 Bookmarklet，工具會直接從目前頁面整理主留言，並在裝置本機完成 CSV 匯出、篩選與抽獎。工具不展開、不保存回覆。
...
4. 點選「FB 留言抽獎」書籤，按「載入全部主留言」。
5. 在「完整主留言」查看、搜尋或下載 CSV 原始留言資料。
6. 設定關鍵字、日期、排除貼文作者、正取及備取人數後開始抽獎。

## 功能

- 半自動展開更多主留言與被截斷的主留言全文，顯示進度並可隨時停止；回覆控制項一律忽略。
- 只保存主留言；抽獎條件不會修改或刪除完整原始主留言資料。
- 取得頁面中可見的姓名、完整主留言內容、個人頁連結及可取得的時間、留言連結與圖片／貼圖資訊。
- 可在工具內搜尋完整主留言並下載 UTF-8 CSV；iPhone Safari 會開啟原生分享面板，可選擇「儲存到檔案」。
- Facebook 顯示總數可能包含回覆，因此主留言模式不拿該數字核對完整度；工具會以兩輪到底且沒有更多主留言控制項作為完成條件。
...
- 只載入、保存、匯出及抽取主留言；回覆不展開、不統計，也不列入候選名單。
- 只能取得使用者目前有權查看、且已載入或工具成功展開的留言。
- Facebook 顯示的留言總數可能包含回覆、無權查看、遭刪除、被系統隱藏或尚未提供到 DOM 的項目，因此只作診斷參考。
